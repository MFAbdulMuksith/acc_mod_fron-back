...
function Income() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('https://accounts.back.opsdemo.xyz/api/income', { customer_id, date, description, amount });
      toast.success('Income added Successfully!!');
      setDate('');
      setDescription('');
      setAmount('');
      axios.get('https://accounts.back.opsdemo.xyz/api/incomelist')
        .then(response => {
  ...
  const handleDelete = (id) => {
    ...
    axios.delete(`https://accounts.back.opsdemo.xyz/api/removeincome/${id}`)
      .then(response => {
